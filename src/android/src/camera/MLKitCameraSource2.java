...
public class MLKitCameraSource2 {
    ...
    private MLKitBarcodeScanningProcessor _ScanningProcessor;
    ...
    public static class Builder {
        ...
        private MLKitCameraSource2 _CameraSource = new MLKitCameraSource2();
        
        public Builder(Context context, MLKitBarcodeScanningProcessor scanningProcessor) {
            if (context == null) {
        ...
        public MLKitCameraSource2 build() {
            _CameraSource._FrameProcessor = _CameraSource.new FrameProcessingRunnable();
            return _CameraSource;
        }
    }
    
    private MLKitCameraSource2() { // Constructor is private to force creation using the builder class.
    }
    ...
    public MLKitCameraSource2 start() throws IOException {
        synchronized (_CameraLock) {
            if (_Camera != null) {
                return this;
            }
    ...
    @RequiresPermission(Manifest.permission.CAMERA)
    public MLKitCameraSource2 start(SurfaceHolder p_SurfaceHolder) throws IOException {
        synchronized (_CameraLock) {
            if (_Camera != null) {
                return this;
            }
            ...
            while (true) {
                ...
                try {
                    synchronized (_CameraLock) {
                        ...
                        _ScanningProcessor.Process(data, _PreviewSize.getHeight(), _PreviewSize.getWidth(), 0);
                        
                        //            Task<List<Barcode>> result = scanner.process(image)
                        //              .addOnSuccessListener(new OnSuccessListener<List<Barcode>>() {
                        //                @Override
                        //                public void onSuccess(List<Barcode> barcodes) {
                        //                  Log.d(TAG, "onSuccess: " + barcodes.toString());
                        //                }
                        //              })
