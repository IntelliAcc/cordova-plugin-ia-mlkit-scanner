...
public class MLKitCameraSourcePreview extends ViewGroup {
  ...
  public MLKitCameraSourcePreview(Context p_Context, AttributeSet p_AttributeSet) {
    ...
    _HorizontalLine.setBackgroundResource(getResources().getIdentifier("mlkit_horizontal_line", "drawable", _Context.getPackageName()));
    addView(_HorizontalLine);

    _VerticalLine = new View(_Context);
    _VerticalLine.setBackgroundResource(getResources().getIdentifier("mlkit_vertical_line", "drawable", _Context.getPackageName()));
    addView(_VerticalLine);

    _TorchButton = new Button(_Context);
    _TorchButton.setBackgroundResource(getResources().getIdentifier("mlkit_torch_inactive", "drawable", _Context.getPackageName()));
      ...
      public void onClick(View v) {
        try {
          ...
          _TorchButton.setBackgroundResource(getResources()
              .getIdentifier(_FlashState ? "mlkit_torch_active" : "mlkit_torch_inactive", "drawable", _Context.getPackageName()));
        } catch (Exception e) {
  ...
  @RequiresPermission(Manifest.permission.CAMERA)
  public void start(MLKitCameraSource2 p_CameraSource, MLKitGraphicOverlay overlay) throws IOException, SecurityException {
    _Overlay = overlay;
